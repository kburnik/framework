...
class MySQLProviderTestModule extends TestModule {
	...
	private function connect() {
		$this->base->connect("localhost","root","",$this->db);
		$connected = true;
	}
	...
	public function testDisconnect() {
		...
		$db = $this->base->execute("select database();")->toCell();
		$this->assertEquality($db,"",true,"Disconnected from database");		
	...
	public function testExportTableStructure() {
		
	}

	public function testCompareTableStructure() {
	...
	public function testAddEventListener() {
		...
		$this->base->addEventListener('onExecute',array($this,'simpleEvent'));		
		
		// test 1
		$query = "select database();";
		$this->base->onExecute($query,null);
	...
	public function testAddEventHandler() {
		$tested_events = array_keys($this->handler->tested_events);
		$all_events = get_class_methods($this->handler);
		
		$diff = array_diff($all_events,$tested_events);		
		
		$this->assertEquality($diff,array(),true);
	}
